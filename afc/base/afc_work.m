...
if ( def.afcwinEnable > 0 )
	...
	if isvalid(def.afc_win) 
		def.afc_win.UserData = 1;									% ready to get answer
		...
		if (work.matlabVersion > 6 )
            ...
            % dont ask why, otherwise 1st button cant get cleared;  THIS
            % PART NO LONGER NECCESSARY IN R2020a (and possibly earlier)
...
%                     set(hb,'enable','on');
		...
		if ( def.modelEnable == 0 ) %&& def.remoteCloneEnable == 0 )
			waitfor(def.afc_win,'Userdata',-1);
