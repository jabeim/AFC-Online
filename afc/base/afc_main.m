...
if isa(varargin{end},'matlab.ui.Figure')     % added block to handle passing the figure window from appdesigner into afc
    afc_win = varargin{end};
...
if exist('afc_win','var')     % added block to handle passing the figure window from appdesigner into afc
    def.afc_win = afc_win;
end
...
%     h=findobj('Tag','afc_win');							% handle to window
    ...
    if ( matlabVersion > 6 )
        ...
        % This dummy control is no longer needed
        %         uicontrol('Parent',h,'position',[-5 -5 0.01 0.01],'Visible','on','enable','on','style','text','Tag','afc_focusdummy','KeyPressFcn',get(h,'KeyPressFcn'));
...
if ( ini < 2 )
    ...
    % JAB 4.2020 deactivated this block for compatability with webapp
...
%         def.afc_win.Visible = 'on';
        if ( def.modelEnable > 0 )
            ...
            waitfor(def.afc_win,'UserData',0);
        end
    end
    ...
    work.elapsedTime = 0;  % added tracking of elapsed time
