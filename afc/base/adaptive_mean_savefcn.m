...
if strcmp(def.externSoundCommand,'webAudio')
    headerl = [headerl '   elapsed time[s]'];
end
%%%%%%%% end headerline %%%%%%%%%
...
for i = 1:tmpinterleavenum
    ...
    if strcmp(def.externSoundCommand,'webAudio')
        res{i}=[mean(restmp) std(restmp,1) work.elapsedTime];
        work.elapsedTime = 0;  % reset the elapsed time after saving
        r='   %8.8f   %8.8f   %8.8f';
    else
