...
y = def.afc_win.UserData;		% ready flag: 0 = not ready, 1 = accept commands,
...
%4/7/2020 removed this for R2020a webapp compatability

% if ( work.matlabVersion > 6 )
%     uicontrol(findobj('Tag','afc_focusdummy'));
% end
figure(def.afc_win);  % testing for webapp
drawnow;
...
elseif x == 0		% called with zero, retrieve currentcharacter
    ...
    if ( ~isempty( def.keyboardResponseButtonMapping ) & ~strcmp(get(h,'currentcharacter'),'s') & ~strcmp(get(h,'currentcharacter'),'e') )
        ...
        % old code
%         x = [];
        x=str2num(def.afc_win.CurrentCharacter);
        if isempty(x) == 1
            x=def.afc_win.CurrentCharacter;
        end
    end
...
%     hb=findobj('Tag',['afc_button' num2str(x)]);
    ...
    def.afc_button(x).BackgroundColor = [.5 .5 .5];
    def.afc_button(x).BackgroundColor = [.75 .75 .75];
...
if ( (work.matlabVersion > 6 ) & (pressedbutton > 0) & isnumeric(pressedbutton))
    figure(def.afc_win); % return focus to the UI window here
%     if (y == 4)
%         % waitfor in main, clear focus from all buttons
%         clearjob = [1 def.intervalnum];
% 
%         % dont ask why, otherwise 1st button cant get cleared, REMOVED
%         % UNNECCESSARY
% %         uicontrol(findobj('Tag','afc_focusdummy'));
%     else
...
%             def.afc_button(clearb).Enable = 'off';
%             drawnow;
%             def.afc_button(clearb).Enable = 'on';
%             drawnow;
%         end
%     end
end
