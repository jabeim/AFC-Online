...
switch ( action )
    ...
    case 'close'
        switch def.externSoundCommand
            ...
            case 'webAudio'
                % add search here for any audio files that have the subj ID
                % matching current session to do final cleanup
            case ''
...
%                     disp('init success')
    ...
    case 'blockWhilePlaying'
        ...
        switch def.externSoundCommand
            ...
            case 'webAudio'
                waitfor(def.webAudioPlayer,'Data',4);
                delete([def.webAudioPath work.audioFileName]);
